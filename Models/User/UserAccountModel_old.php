	...
	class UserAccountModel extends DbConnectSql{
		/* ▂ ▅ Methodes ▅ ▂ */
			...
			/* ▂▂▂▂▂▂▂▂▂▂▂ */
				...
				/* ▂ ▅  findId( int $id )  ▅ ▂ */
				public function findJointId( int $id ) { 
					$this -> request = $this -> connexion -> prepare( "SELECT useraccount.*, loginaccount.* FROM useraccount INNER JOIN loginaccount ON useraccount.idUserAccount = loginaccount.idUserAccount WHERE useraccount.idUserAccount=:idUserAccount" );
					$this -> request -> bindParam(":idUserAccount", $id , PDO::PARAM_STR);
					$this -> request -> execute();
					$results = $this -> request -> fetch(PDO::FETCH_OBJ);
					return $results;
